...
A user-writable checkout is never a trusted root execution source. The host operator MUST NOT execute `install/install-id-exergism.sh` directly with sudo. Before any repository shell is parsed as root, the reviewed installer bytes are copied with system tools into a root-owned mode-0700 stage below `/var/lib/ec-deployment-attestation/bootstrap`, whose filesystem must permit execution, checked against an out-of-band reviewed `EC_INSTALLER_SHA256`, and only that mode-0500 staged copy is executed. `/run` remains suitable for locks and ephemeral state and may remain mounted `noexec`. The staged installer treats `EC_INSTALLER_SOURCE_ROOT` as untrusted input: every repository helper, unit, schema, example and smoke script consumed from it must match the SHA-256 values embedded in the already-authenticated installer. The Native AOT candidate is likewise accepted only when its bytes match the independently reviewed `EC_NATIVE_AGENT_SHA256`.
